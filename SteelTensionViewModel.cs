...
{
    public class SteelTensionViewModel : INotifyPropertyChanged
    {
        ...
        private double _fu = 65.0;
        private int _methodIndex = 0;
        private double _tu = 100.0;
        private double _memberLength = 0;

        private int _connectionCaseIndex = 0;
        private double _connectionLength = 0;
        private double _xbar = 0;
        private double _userU = 1.0;

        private bool _useAutoNetArea = true;
        private double _an = 0;
        private double _boltHoleDiameter = 0.875;
        private int _numberOfHoles = 2;
        ...
        {
            public string Name = "";
            public SteelTensionCalculations.SectionShapeType ShapeType;
            public SteelTensionCalculations.SectionProperties Properties = new();
        }
        ...
        public SteelTensionViewModel()
        {
            ...
            ConnectionCaseOptions = new List<string>
            {
                "Case 1: All elements connected (U=1.0)",
                "Case 2/7: Flange bolted, >=3 fasteners (bf/d + Case 2)",
                "Case 7c: Web bolted, >=4 fasteners (U=0.70)",
                "Case 6a: HSS single gusset plate",
                "Case 6b: HSS two side gusset plates",
                "User-defined U"
            };
            LoadShapeDatabase();
        }
        ...
        public List<string> ConnectionCaseOptions { get; }
        ...
        // Custom I-shape dimensions
        public double CustomD
        {
            get => _customD;
            set { _customD = value; OnPropertyChanged(); }
        }
        public double CustomBf
        {
            get => _customBf;
            set { _customBf = value; OnPropertyChanged(); }
        }
        public double CustomTw
        {
            get => _customTw;
            set { _customTw = value; OnPropertyChanged(); }
        }
        public double CustomTf
        {
            get => _customTf;
            set { _customTf = value; OnPropertyChanged(); }
        }

        // Custom HSS dimensions
        public double CustomHt
        {
            get => _customHt;
            set { _customHt = value; OnPropertyChanged(); }
        }
        public double CustomB
        {
            get => _customB;
            set { _customB = value; OnPropertyChanged(); }
        }
        public double CustomT
        {
            get => _customT;
            set { _customT = value; OnPropertyChanged(); }
        }
        ...
        {
            get => _fu;
            set { _fu = value; OnPropertyChanged(); }
        }
        ...
        {
            get => _tu;
            set { _tu = value; OnPropertyChanged(); }
        }
        public double MemberLength
        {
            get => _memberLength;
            set { _memberLength = value; OnPropertyChanged(); }
        }

        // Connection parameters
        public int ConnectionCaseIndex
        {
            get => _connectionCaseIndex;
            set
            {
                _connectionCaseIndex = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(ShowConnectionLength));
                OnPropertyChanged(nameof(ShowXbar));
                OnPropertyChanged(nameof(ShowUserU));
                OnPropertyChanged(nameof(ShowBoltHoles));
            }
        }

        public bool ShowConnectionLength => _connectionCaseIndex == 1 || _connectionCaseIndex == 3 || _connectionCaseIndex == 4;
        public bool ShowXbar => _connectionCaseIndex == 1;
        public bool ShowUserU => _connectionCaseIndex == 5;
        public bool ShowBoltHoles => _connectionCaseIndex != 5;

        public double ConnectionLength
        {
            get => _connectionLength;
            set { _connectionLength = value; OnPropertyChanged(); }
        }
        public double Xbar
        {
            get => _xbar;
            set { _xbar = value; OnPropertyChanged(); }
        }
        public double UserU
        {
            get => _userU;
            set { _userU = value; OnPropertyChanged(); }
        }

        // Net area parameters
        public bool UseAutoNetArea
        {
            get => _useAutoNetArea;
            set
            {
                _useAutoNetArea = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(ShowBoltHoleInputs));
                OnPropertyChanged(nameof(ShowManualAn));
            }
        }
        public bool ShowBoltHoleInputs => _useAutoNetArea;
        public bool ShowManualAn => !_useAutoNetArea;

        public double An
        {
            get => _an;
            set { _an = value; OnPropertyChanged(); }
        }
        public double BoltHoleDiameter
        {
            get => _boltHoleDiameter;
            set { _boltHoleDiameter = value; OnPropertyChanged(); }
        }
        public int NumberOfHoles
        {
            get => _numberOfHoles;
            set { _numberOfHoles = value; OnPropertyChanged(); }
        }
        ...
        {
            get => _tensionStatus;
            set { _tensionStatus = value; OnPropertyChanged(); }
        }
        ...
        private void LoadFromCsv(string csvPath)
        {
            foreach (var line in File.ReadLines(csvPath).Skip(1))
            {
                ...
                var props = new SteelTensionCalculations.SectionProperties
                {
                    ShapeType = type == "W"
                        ? SteelTensionCalculations.SectionShapeType.IShape
                        : SteelTensionCalculations.SectionShapeType.HSS,
                    Name = f[1].Trim(),
                    A = D(f[2]),
        ...
        private void SaveToCsv(string csvPath)
        {
            ...
            foreach (var s in _allShapes)
            {
                ...
                sw.WriteLine(string.Join(",",
                    s.ShapeType == SteelTensionCalculations.SectionShapeType.IShape ? "W" : "HSS",
                    p.Name, F(p.A),
        ...
        private SteelTensionCalculations.SectionProperties? GetSelectedSection()
        {
            if (IsDatabase)
            {
            ...
            if (IsIShape)
            {
                if (_customD <= 0 || _customBf <= 0 || _customTw <= 0 || _customTf <= 0)
                    return null;
                return SteelTensionCalculations.ComputeCustomIShape(_customD, _customBf, _customTw, _customTf);
            }
            else
            {
                if (_customHt <= 0 || _customB <= 0 || _customT <= 0)
                    return null;
                return SteelTensionCalculations.ComputeCustomHSS(_customHt, _customB, _customT);
            }
        }

        private SteelTensionCalculations.ConnectionCase GetConnectionCase()
        {
            return _connectionCaseIndex switch
            {
                0 => SteelTensionCalculations.ConnectionCase.AllElementsConnected,
                1 => SteelTensionCalculations.ConnectionCase.IShapeFlangeBolted,
                2 => SteelTensionCalculations.ConnectionCase.IShapeWebBolted,
                3 => SteelTensionCalculations.ConnectionCase.HSSSingleGusset,
                4 => SteelTensionCalculations.ConnectionCase.HSSTwoSideGusset,
                5 => SteelTensionCalculations.ConnectionCase.UserDefined,
                _ => SteelTensionCalculations.ConnectionCase.AllElementsConnected
            };
        }
        ...
        private void Calculate()
        {
            try
            {
                ...
                var input = new SteelTensionCalculations.InputParameters
                {
                    Section = section,
                    Fy = _fy,
                    Fu = _fu,
                    Tu = _tu,
                    MemberLength = _memberLength,
                    Method = _methodIndex == 0
                        ? SteelTensionCalculations.DesignMethod.LRFD
                        : SteelTensionCalculations.DesignMethod.ASD,
                    ConnectionType = GetConnectionCase(),
                    ConnectionLength = _connectionLength,
                    Xbar = _xbar,
                    UserU = _userU,
                    UseAutoNetArea = _useAutoNetArea,
                    An = _an,
                    BoltHoleDiameter = _boltHoleDiameter,
                    NumberOfHoles = _numberOfHoles
                };

                var result = SteelTensionCalculations.Calculate(input);
                ...
                Ratio = result.Ratio;
                IsOK = result.IsOK;
                TensionStatus = result.IsOK ? "PASS" : "FAIL";
