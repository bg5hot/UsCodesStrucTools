...
{
    public class SteelBeamViewModel : INotifyPropertyChanged
    {
        ...
        private double _mu = 100.0;
        private double _vu = 50.0;
        private double _lb = 10.0;
        private double _cb = 1.0;
        ...
        {
            public string Name = "";
            public SteelBeamCalculations.SectionShapeType ShapeType;
            public SteelBeamCalculations.SectionProperties Properties = new();
        }
        ...
        public SteelBeamViewModel()
        {
            CalculateCommand = new RelayCommand(Calculate);
            SectionTypeOptions = new List<string> { "I-Shape (W)", "HSS (Rectangular/Square)" };
        ...
        {
            get => _cb;
            set { _cb = value; OnPropertyChanged(); }
        }
        ...
        public double FlexuralRatio
        {
            get => _flexuralRatio;
            set { _flexuralRatio = value; OnPropertyChanged(); }
        }
        ...
        {
            get => _flexuralOK;
            set { _flexuralOK = value; OnPropertyChanged(); }
        }
        public bool ShearOK
        {
            get => _shearOK;
            set { _shearOK = value; OnPropertyChanged(); }
        }
        ...
        {
            get => _flexuralStatus;
            set { _flexuralStatus = value; OnPropertyChanged(); }
        }
        public string ShearStatus
        {
            get => _shearStatus;
            set { _shearStatus = value; OnPropertyChanged(); }
        }
        ...
        // CSV column order (must match LoadFromCsv / SaveToCsv):
        // Type,Name,A,d,bf,tw,tf,h,Ht,B,tdes,b_flat,Ix,Iy,Sx,Sy,Zx,Zy,rx,ry,J,Cw,rts,ho
        ...
        private void LoadFromCsv(string csvPath)
        {
            foreach (var line in File.ReadLines(csvPath).Skip(1)) // skip header
            {
                ...
                var props = new SteelBeamCalculations.SectionProperties
                {
                    ShapeType = type == "W"
                        ? SteelBeamCalculations.SectionShapeType.IShape
                        : SteelBeamCalculations.SectionShapeType.HSS,
                    Name = f[1].Trim(),
                    A = D(f[2]),
        ...
        private void SaveToCsv(string csvPath)
        {
            ...
            foreach (var s in _allShapes)
            {
                ...
                sw.WriteLine(string.Join(",",
                    s.ShapeType == SteelBeamCalculations.SectionShapeType.IShape ? "W" : "HSS",
                    p.Name, F(p.A),
        ...
        private SteelBeamCalculations.SectionProperties? GetSelectedSection()
        {
            if (IsDatabase)
            {
                int idx = _selectedShapeIndex - 1; // offset for "-- Select --"
            ...
            if (IsIShape)
            {
                if (_customD <= 0 || _customBf <= 0 || _customTw <= 0 || _customTf <= 0)
                    return null;
                return SteelBeamCalculations.ComputeCustomIShape(_customD, _customBf, _customTw, _customTf);
            }
            else
            {
                if (_customHt <= 0 || _customB <= 0 || _customT <= 0)
                    return null;
                return SteelBeamCalculations.ComputeCustomHSS(_customHt, _customB, _customT);
            }
        }
        ...
        private void Calculate()
        {
            try
            {
                ...
                var input = new SteelBeamCalculations.InputParameters
                {
                    Section = section,
                    Fy = _fy,
                    E = _e,
                    ...
                    Method = _methodIndex == 0
                        ? SteelBeamCalculations.DesignMethod.LRFD
                        : SteelBeamCalculations.DesignMethod.ASD
                };

                var result = SteelBeamCalculations.Calculate(input);
                ...
                FlexuralStatus = result.FlexuralOK ? "PASS" : "FAIL";
                ShearStatus = result.ShearOK ? "PASS" : "FAIL";
